...
def test_change_username(client, normal_user_token_headers):
    ...
    response = client.put(
        "/username", json.dumps(data), headers=normal_user_token_headers
    )
    assert response.status_code == 200
...
def test_change_email_with_existing_email(client, normal_user_token_headers):
    data = {"email": "testing@reminderse.com"}
    response = client.put("/email", json.dumps(data), headers=normal_user_token_headers)
    assert response.status_code == 400
...
def test_change_password(client, normal_user_token_headers):
    data = {"password": "test1"}
    response = client.put(
        "/password", json.dumps(data), headers=normal_user_token_headers
    )
    assert response.status_code == 200
    assert response.json()["message"] == "Changes saved"
    data = {"email": "testing@reminderse.com", "password": data.get("password")}
...
def test_change_interval(client, normal_user_token_headers):
    data = {"interval": "5"}
    response = client.put(
        "/interval", json.dumps(data), headers=normal_user_token_headers
    )
    assert response.status_code == 200
    assert response.json()["message"] == "Changes saved"
...
def test_token_required(client):
    protected_endpoints = ["/username", "/email", "/password", "/interval"]
    data = {}

    for endpoint in protected_endpoints:
        if endpoint == "/username":
            data = {"username": "bingus"}
        elif endpoint == "/email":
            data = {"email": "test1@reminderse.com"}
        ...
        assert (
            response.json()["detail"] == "Missing Authorization Header"
            or response.json()["detail"] == "Missing cookie access_token_cookie"
        )
