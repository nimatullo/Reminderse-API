...
from core.entries.repo.repo import LinkRepo, TextRepo, get_category_by_id
from core.entries.repo.repo import get_category_by_id, get_category_by_title, category_exists, add_new_category
...
class EntryService:
    ...
    def add_link(self, entry_title, url, category_title, date_of_next_send=None):
        """
        Cases:
        ...
        if category_title:
            ...
            if self.link_repo.add(entry_title, validated_url, category.id, date_of_next_send):
                return make_response(jsonify({
            ...
            if self.link_repo.add(entry_title, validated_url, date_of_next_send):
                return make_response(jsonify({
                    "message": "Link entry created"
                }), 201)
            else:
    ...
    def add_text(self, entry_title, content, category_title, date_of_next_send=None):
        if category_title:
            category = category_exists(category_title)
            if not category:
                category = add_new_category(category_title)

            if self.text_repo.add(entry_title, content, category.id, date_of_next_send):
                return make_response(jsonify({
            ...
            if self.text_repo.add(title=entry_title, content=content, date_of_next_send=date_of_next_send):
                return make_response(jsonify({
                    "message": "Text entry created"
                }), 201)
            else:
